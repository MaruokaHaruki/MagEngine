...
namespace MagEngine {
	...
	void LightManager::Initialize() {
		...
		mainSpotLight.cosFalloffStart = cosf(defaultAngle * 0.7f); // 内側の角度
		mainSpotLight.cosFalloffEnd = cosf(defaultAngle);		   // 外側の角度
	...
	void LightManager::DrawImGui() {
		...
		if (ImGui::CollapsingHeader("Directional Lights", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (ImGui::Combo("Active Directional Light", &selectedIndex, items)) {
				...
				case 0:
					SetActiveDirectionalLight("Main");
					break;
				case 1:
					SetActiveDirectionalLight("Custom1");
					break;
				case 2:
					SetActiveDirectionalLight("Custom2");
					break;
				}
			}
		...
		if (ImGui::CollapsingHeader("Point Lights", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (ImGui::Combo("Active Point Light", &selectedIndex, items)) {
				...
				switch (selectedIndex) {
				case 0:
					SetActivePointLight("Main");
					break;
				case 1:
					SetActivePointLight("Custom1");
					break;
				case 2:
					SetActivePointLight("Custom2");
					break;
				}
			}
		...
		if (ImGui::CollapsingHeader("Spot Lights", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (ImGui::Combo("Active Spot Light", &selectedIndex, items)) {
				...
				case 0:
					...
					break;
				case 1:
					SetActiveSpotLight("Custom1");
					break;
				case 2:
					SetActiveSpotLight("Custom2");
					break;
				}
			}
			...
			// 外側の角度からラジアンに変換
			float angleRad = acosf(light.cosFalloffEnd);
			...
			if (ImGui::SliderFloat("Angle (degrees)##SpotLight", &angleDeg, 0.0f, 90.0f)) {
				...
				light.cosFalloffStart = cosf(rad * 0.7f); // 内側の角度
				light.cosFalloffEnd = cosf(rad);		  // 外側の角度
			}
		}
	...
	void LightManager::AddSpotLight(const std::string &name, const MagMath::Vector4 &color,
		...
		light.decay = decay;
		light.cosFalloffStart = cosf(angle * 0.7f); // 内側の角度
		light.cosFalloffEnd = cosf(angle);			// 外側の角度
	...
	void LightManager::VisualizeSpotLight(const std::string &lightName) {
		...
		// 光の角度（コサイン値から角度に変換）外側の角度を使用
		float angle = acosf(light.cosFalloffEnd);
