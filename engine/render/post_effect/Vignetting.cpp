...
namespace MagEngine {
	...
	///=============================================================================
	///                        パイプラインの作成
	PipelineRecipe Vignetting::CreateDefaultRecipe(ID3D12RootSignature *rootSignature, const std::wstring &vertexShaderPath, const std::wstring &pixelShaderPath) {
	...
	PipelineRecipe Vignetting::CreatePipelineRecipe() const {
		return CreateDefaultRecipe(rootSignature_.Get(), vertexShaderPath_, pixelShaderPath_);
	}

	void Vignetting::CreatePipeline() {
		CreateRootSignature();
		// NOTE: Ping-PongやDescriptor切り替えはManager側に残し、PSO生成の定型処理だけBuilderへ委譲する。
