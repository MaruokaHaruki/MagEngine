...
///=============================================================================
///                        初期化
	void FullscreenPassRendere::Initialize(DirectXCore *dxCore) {
		//========================================
		// シェーダーパスの設定
		vertexShaderPath_ = L"resources/shader/FullScreen.VS.hlsl";
		pixelShaderPath_ = L"resources/shader/Fullscreen.PS.hlsl";
		//=======================================
		// DirectXCoreのポインタを取得
		dxCore_ = dxCore;
		// パイプラインの作成
	...
	///=============================================================================
	///                        描画
	void FullscreenPassRendere::PreDraw() {
		//========================================
		// 以下の内容を繰り返す
	...
	///=============================================================================
	///                        描画後処理
	void FullscreenPassRendere::PostDraw() {
	}

	///=============================================================================
	///                        パイプラインの作成
	PipelineRecipe FullscreenPassRendere::CreateDefaultRecipe(ID3D12RootSignature *rootSignature, const std::wstring &vertexShaderPath, const std::wstring &pixelShaderPath) {
	...
	PipelineRecipe FullscreenPassRendere::CreatePipelineRecipe() const {
		return CreateDefaultRecipe(rootSignature_.Get(), vertexShaderPath_, pixelShaderPath_);
	}

	void FullscreenPassRendere::CreatePipeline() {
		CreateRootSignature();
		// NOTE: Fullscreen描画のDescriptorとDrawは呼び出し側に残し、PSO生成の定型処理だけBuilderへ委譲する。
	...
	///=============================================================================
	///                        ルートシグネチャの作成
	void FullscreenPassRendere::CreateRootSignature() {
		HRESULT hr;
		//========================================
		// ルートシグネチャの設定
