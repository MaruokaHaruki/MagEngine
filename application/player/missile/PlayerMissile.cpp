 ...
 *         2025-01-25: MagMath関数に統一（Normalize, Dot等）
 *         従前のNormalizeVector, DotProduct関数を廃止
 *********************************************************************/
...
void PlayerMissile::Initialize(
	...
	forward_ = MagMath::Normalize(direction);
	speed_ = 50.0f;		   // 一定速度
...
void PlayerMissile::UpdateTracking() {
	...
	if (target_ && target_->IsAlive() && trackingStrength_ > 0.01f) {
		...
		if (distance > 0.1f) {
			isTracking_ = true;
			Vector3 targetDirection = MagMath::Normalize(toTarget);
			...
			if (isLockedOn_ || lifetime_ > desiredHitTime_ * 0.7f) {
				...
				// 通常は滑らかに向きを変える
				float dotProduct = MagMath::Dot(forward_, targetDirection);
			...
			forward_ = MagMath::Normalize(forward_);
			}
		}
	}
}
