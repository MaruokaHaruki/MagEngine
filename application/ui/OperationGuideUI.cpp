...
void OperationGuideUI::InitializeButtons() {
	...
	buttonRT.sprite->Initialize(spriteSetup_, "xbox_rt.dds");
	...
	buttonB.sprite->Initialize(spriteSetup_, "xbox_button_color_b.dds");
	...
	buttonA.sprite->Initialize(spriteSetup_, "xbox_button_color_a.dds");
	...
	buttonY.sprite->Initialize(spriteSetup_, "xbox_button_color_y.dds");
...
///=============================================================================
///                        展開アニメーション開始
void OperationGuideUI::StartDeployAnimation(float duration) {
	isAnimating_ = true;
	isDeploying_ = true;
	deployAnimationTime_ = 0.0f;
	deployAnimationDuration_ = duration;
	deployProgress_ = 0.0f;
	baseOpacity_ = opacity_;
	baseGuidePosition_ = guideBasePosition_;
}

///=============================================================================
///                        収束アニメーション開始
void OperationGuideUI::StartRetractAnimation(float duration) {
	isAnimating_ = true;
	isDeploying_ = false;
	deployAnimationTime_ = 0.0f;
	deployAnimationDuration_ = duration;
	deployProgress_ = 1.0f;
	baseOpacity_ = opacity_;
	baseGuidePosition_ = guideBasePosition_;
}
...
void OperationGuideUI::UpdateDeployAnimation() {
	...
	float rawProgress = deployAnimationTime_ / deployAnimationDuration_;

	if (isDeploying_) {
		// 展開アニメーション：EaseOutCubic を使用
		deployProgress_ = EaseOutElastic(rawProgress);

		// 不透明度を段階的に上げる
		opacity_ = baseOpacity_ * deployProgress_;

		// 位置を右から左へスライドイン
		float slideAmount = -100.0f * (1.0f - deployProgress_);
		guideBasePosition_.x = baseGuidePosition_.x + slideAmount;
	} else {
		// 収束アニメーション：逆再生
		deployProgress_ = 1.0f - EaseOutElastic(rawProgress);
